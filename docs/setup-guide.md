...
Define this in both `.env.example` and `.env`:
...
The application is accessed from the host through Nginx on:

- `127.0.0.1:${NGINX_PORT}`

Jenkins is accessed from the host on:
...
Because the project supports blue/green deployment and rollback, migration design must remain conservative.
...
As of Phase 10, the preferred local runtime is Docker Compose with:
...
### 6. Bring up the app stack without leaving orphans
...
At the current phase, the host should access the API through Nginx, not directly through the app containers.
...
The current application runtime includes:
...
### Validate public traffic after the switch using GET
...
### Simulate a controlled deploy failure
...
- the target color fails healthcheck
- traffic does not move
- the previous active color remains serving through Nginx

### Recover the inactive color after a failed simulation
...
At the current phase, Jenkins executes a real CI pipeline from the repository `Jenkinsfile` and can continue into local blue/green deployment.
...
- controlled deploy-failure simulation aborts traffic switching cleanly
...
```bash
docker compose stop postgres app_blue app_green nginx jenkins-controller jenkins-agent
```
...
The API should be accessed from the host through Nginx:
...
Jenkins should be accessed from the host through:
...
- controlled deploy failure kept the previous color serving
...
Operational references are now available under:
